...
const SellRequestsPage = () => {
  ...
  const fetchOrders = async () => {
    const res = await axios.get(`/admin/sell-orders?search=${search}&page=${page}`);
                ...
                <td className="p-3 text-right relative">
                  ...
                  {dropdownOpen === order._id && (
                    <div className="absolute right-0 mt-2 w-40 bg-white border rounded shadow z-10 text-black">
                      ...
                      {order.status === 'approved' && order.remainingAmount > 0 && (<button onClick={() => handleAction('match', order)} className="w-full px-4 py-2 hover:bg-gray-100 text-left">Match</button>)}
                    </div>
                  )}
                </td>
              </tr>
